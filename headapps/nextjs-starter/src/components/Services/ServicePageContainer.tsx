...
const DefaultServicePageContainer = (props: ComponentProps): JSX.Element => {
  const containerStyles = props?.params?.Styles ?? '';
  const styles = `${props?.params?.GridParameters} ${containerStyles}`.trimEnd();
  const phKey = `service-page-container-${props?.params?.DynamicPlaceholderId}`;
  ...
  if (splitStyles && splitStyles.includes('container')) {
    return (
      <div className="container-wrapper">
        <DefaultServicePageContainer {...props} />
      </div>
    );
  }

  return <DefaultServicePageContainer {...props} />;
};
